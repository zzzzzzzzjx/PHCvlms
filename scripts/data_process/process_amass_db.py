...
def process_qpos_list(qpos_list):
    ...
    for (k, v) in tqdm(pbar):
        ...
        # 兼容不同 AMASS 版本的帧率字段命名：
        # 有的叫 "mocap_framerate"，有的叫 "mocap_frame_rate"，还有可能完全没有（fallback=30）
        # 兼容不同 AMASS 版本的帧率字段命名：
        if "mocap_framerate" in v:
            amass_fr = v["mocap_framerate"]
        elif "mocap_frame_rate" in v:
            amass_fr = v["mocap_frame_rate"]
        else:
            amass_fr = 30.0  # 默认 30fps

        if hasattr(amass_fr, "item"):
            amass_fr = amass_fr.item()

        skip = int(amass_fr / target_fr)

        # 兼容不同的姿态字段命名：
        # 标准 AMASS 有 "poses"
        # 某些 stageII/retargeted 版本只有 root_orient + pose_body
        if "poses" in v:
            amass_pose = v["poses"][::skip]
        elif "pose_body" in v and "root_orient" in v:
            root = v["root_orient"][::skip]      # [N, 3]
            body = v["pose_body"][::skip]        # [N, 63]
            # 简单拼成 [root | body]，忽略手/眼/下颚，只用主要身体关节
            amass_pose = np.concatenate([root, body], axis=1)
        else:
            print(f"[WARN] no pose field for {k}, keys = {list(v.keys())}, skip this seq")
            removed_k.append(k)
            continue

        # trans 字段名一般是一致的，如果不存在也跳过
        if "trans" not in v:
            print(f"[WARN] no trans field for {k}, skip this seq")
            removed_k.append(k)
            continue
