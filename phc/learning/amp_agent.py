...
#         self.wandb_run = None
#         try:
#             # project/name 你可以之后在 hydra config 里专门配，
#             # 目前先用一个比较直观的默认值
#             project_name = config.get("wandb_project", "phc_kungfu")
#             run_name = config.get("wandb_run_name", base_name)

#             # 把整个 config 传进去，方便以后在网页上查看超参
#             self.wandb_run = wandb.init(
#                 project=project_name,
#                 name=run_name,
#                 config=config,
#             )
#             print(f"[W&B] Initialized run: project={project_name}, name={run_name}")
#         except Exception as e:
#             # 如果初始化失败，不影响训练，只是打印一个 warning
#             print(f"[W&B] Failed to init wandb, continue training without W&B. Error: {e}")
#             self.wandb_run = None
#         # ==================================
...
#                     self._task_reward_w = 0.5   # 任务奖励权重
#                     self._disc_reward_w = 0.5   # AMP 判别器奖励权重
#                 else:
...
#                       'prev_actions': actions_batch, 'obs': obs_batch_processed, 'amp_obs': amp_obs,
#                       'amp_obs_replay': amp_obs_replay, 'amp_obs_demo': amp_obs_demo, \
#                       "obs_orig": obs_batch
#                       }
...
#         # 基础对抗损失
#         # prediction loss
#         disc_loss_agent = self._disc_loss_neg(disc_agent_logit) # 把AI动作标记为"假" 训练判别器说"这是假的"（标签=0）
#         disc_loss_demo = self._disc_loss_pos(disc_demo_logit)   # 把真人动作标记为"真" 训练判别器说"这是真的"（标签=1）

#         disc_loss = 0.5 * (disc_loss_agent + disc_loss_demo)

#         # logit reg logit正则化（评委"别太武断，保持谦逊"。）
...
#         # grad penalty 梯度惩罚
#             # 问题：传统GAN训练不稳定，经常"崩掉"
#             # 解决方案：限制判别器的"变化速度"（梯度范数）
#             # 比喻：就像告诉评委"评分标准不要变化太快，要平稳一些"
...
#         with torch.no_grad():
#             disc_logits = self._eval_disc(amp_obs)  # 获取判别器输出的 logits（判别器评分）
#             prob = 1 / (1 + torch.exp(-disc_logits))    # 把 logits 转换成概率（分数归一化）
#             disc_r = -torch.log(torch.maximum(1 - prob, torch.tensor(0.0001, device=self.ppo_device)))
#             # 奖励归一化：
#                 # 计算奖励（奖励 = -log(1 - 概率)）
#                 # 如果prob很高（很像真人），这个值就很小——AI动作90%像真人（prob=0.9）：
#             if (self._norm_disc_reward()):      # 如果需要，则对奖励进行归一化
#                 self._disc_reward_mean_std.train()
#                 norm_disc_r = self._disc_reward_mean_std(disc_r.flatten())
#                 disc_r = norm_disc_r.reshape(disc_r.shape)
#                 disc_r = 0.5 * disc_r + 0.25    # 把归一化的奖励乘以0.5，再加0.25（重新映射到合理范围）

#             disc_r *= self._disc_reward_scale    # 最终缩放：最后乘以奖励缩放因子（默认1.0）（放大奖励）
...
#         # ====== 新增：把本 epoch 的所有指标同步到 W&B ======
#         if self.wandb_run is not None:
#             try:
#                 # 使用 frame 作为 step，这样 X 轴是全局 frame 数，更符合 RL 习惯
#                 wandb.log(train_info_dict)
#             except Exception as e:
#                 # 避免网络问题直接把训练搞崩
#                 print(f"[W&B] wandb.log failed at frame {frame}, error: {e}")
#         # ===================================================
