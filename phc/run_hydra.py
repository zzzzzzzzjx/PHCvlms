...
def create_rlgpu_env(**kwargs):
    ...
    if vlm_enabled and hasattr(task, 'load_vlm_weights'):
        weight_file_raw = cfg.env.get('vlm_weight_file', '')
        weight_file = str(weight_file_raw).strip() if weight_file_raw is not None else ''
        ...
        if len(weight_file) > 0:
            ...
            # 未提供全局文件，若已加载 per-motion 权重则跳过禁用
            if getattr(task, "vlm_motion_weight_buffers", None):
                print("ⓘ VLM per-motion weights detected, skipping global weight file load.")
            else:
                print("⚠️  Warning: VLM weight enabled but no weight file specified")
                task.vlm_weight_enabled = False
    else:
        # 未启用或没有 load_vlm_weights：如果已有 per-motion 权重则跳过，否则按原逻辑提示
        if getattr(task, "vlm_motion_weight_buffers", None):
            print("ⓘ VLM per-motion weights detected, skipping global weight file load.")
        elif vlm_enabled:
            print("⚠️  Warning: VLM weight enabled but no weight file specified")
