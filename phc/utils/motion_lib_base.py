...
class MotionLibBase():
    ...
    def load_data(self, motion_file,  min_length=-1, im_eval = False):
        if isinstance(motion_file, (list, tuple, np.ndarray)):
            self.mode = MotionlibMode.file_list
            data_list = list(motion_file)
            self._motion_data_list = np.array(data_list)
            self._motion_data_keys = np.array(data_list)
            self._num_unique_motions = len(self._motion_data_list)
            # 保持与 file/directory 模式一致，缓存一个示例的加载结果以兼容 real_traj 分支
            self._motion_data_load = joblib.load(data_list[0]) if len(data_list) > 0 else {}
            return
    ...
    def load_motions(self, skeleton_trees, gender_betas, limb_weights, random_sample=True, start_idx=0, max_len=-1):
        ...
        # 简洁日志：仅输出采样到的唯一动作及数量
        unique_ids, counts = torch.unique(sample_idxes, return_counts=True)
        unique_keys = self._motion_data_keys[unique_ids.cpu().numpy()]
        preview_n = min(len(unique_keys), 10)
        print("\n****************************** Current motion keys ******************************")
        print(f"Sampling {len(unique_keys)} unique motions (top {preview_n} shown):")
        for i in range(preview_n):
            print(f"  {i}: {unique_keys[i]} (count={counts[i].item()})")
        if len(unique_keys) > preview_n:
            print("  ...")
        print("*********************************************************************************\n")
