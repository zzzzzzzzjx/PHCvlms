...
class MotionLibSMPL(MotionLibBase):
    ...
    def load_motion_with_skeleton(ids, motion_data_list, skeleton_trees, shape_params, mesh_parsers, config, queue, pid):
        ...
        for f in pbar:
            ...
            if not isinstance(curr_file, dict) and osp.isfile(curr_file):
                key = motion_data_list[f].split("/")[-1].split(".")[0]
                data_loaded = joblib.load(curr_file)
                if isinstance(data_loaded, dict):
                    if key in data_loaded:
                        curr_file = data_loaded[key]
                    elif "pose_quat_global" in data_loaded:
                        # 兼容直接保存单个动作字典的情况
                        curr_file = data_loaded
                    elif len(data_loaded) == 1:
                        # 若只有一个条目，则取唯一 value
                        curr_file = next(iter(data_loaded.values()))
                    else:
                        raise KeyError(f"Key '{key}' not found in {curr_file}. Available keys: {list(data_loaded.keys())}")
                else:
                    curr_file = data_loaded
            curr_gender_beta = shape_params[f]
