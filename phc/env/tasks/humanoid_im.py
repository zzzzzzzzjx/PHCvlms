...
#         Args:
#             env_ids: 需要更新的环境ID列表，None表示更新所有环境
#         """
#         if not self.vlm_weight_enabled or self.vlm_weight_buffer is None:
#             return
...
#         # 计算当前帧索引（循环播放）
#         total_frames = len(self.vlm_weight_buffer)
#         current_frames = (self.progress_buf[env_ids] % total_frames).long()
        
#         # 批量更新权重（避免逐帧更新开销）
#         self.vlm_body_weights[env_ids] = self.vlm_weight_buffer[current_frames] * self.vlm_weight_scale
            
#         # ========== 调试输出 ==========
#         if self.common_step_counter % 500 == 0:  # 每500步输出一次
#             avg_weights = self.vlm_body_weights.mean(dim=0)
#             current_frame = current_frames[0].item()
            
#             print(f"Step {self.common_step_counter}: Frame {current_frame}")
#             print(f"  Avg Weights - Waist:{avg_weights[0]:.3f}, "
#                 f"LHand:{avg_weights[1]:.3f}, RHand:{avg_weights[2]:.3f}, "
#                 f"LFoot:{avg_weights[3]:.3f}, RFoot:{avg_weights[4]:.3f}")
            
#         # 记录当前帧（用于调试）
#         self.current_vlm_frame[env_ids] = current_frames
#    #   ==========================VLM权重增强配置 2==================================
...
#         # 默认权重：所有部位权重为1
#         joint_weights = torch.ones_like(body_pos[..., 0])
#     else:
#         # 扩展权重到每个关节 [num_envs, 5] -> [num_envs, num_joints]
#         num_joints = body_pos.shape[1]
#         joint_weights = torch.ones((body_pos.shape[0], num_joints),
